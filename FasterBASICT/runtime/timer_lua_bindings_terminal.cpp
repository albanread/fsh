#include "timer_lua_bindings_terminal.h"
...
// Global timer system instance for terminal mode
...
void registerTimerBindings(lua_State* L) {
    ...
    // Frame-based functions (AFTERFRAMES, EVERYFRAME) are NOT registered
...
int lua_basic_timer_check(lua_State* L) {
    // NOTE: This function is deprecated and does nothing in terminal mode.
    // Event handling is done entirely by the _event_checker coroutine in the
    // generated Lua code, which calls basic_timer_try_dequeue() to poll events.
    // This function exists only for API compatibility with older code.
    lua_pushinteger(L, 0);
    return 1;
}
...
// C++ utility function to stop all timers from shell (e.g., on Control+C)
void stopAllTimersFromShell() {
    if (!g_timerSystemInitialized || !g_timerManager) {
        return;  // Nothing to do if timer system not initialized
    }
    ...
    try {
        ...
        std::cerr << "Error stopping timers from shell: " << e.what() << std::endl;
    }
}
